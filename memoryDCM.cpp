...
double TE = 0.0; //most recent echo time recorded
	...
	while ((d.imageStart == 0) && ((lPos+8+lFileOffset) <  fileLen)) {
    	...
    	if ((size_t)(lPos + 128) > MaxBufferSz) { //avoid overreading the file
			...
			filemem.read((char *)buffer, MaxBufferSz);
			if (!filemem) {
         		    printError("Not enough bytes laoded \n");
         		return d;
    		}
			lPos = 0;
    	}
        ...
        switch ( groupElement ) {
            ...
            */
            ...
            case kSpectroscopyData: //kSpectroscopyDataPointColumns
            	printMessage("Skipping Spectroscopy DICOM \n");
                d.imageStart = (int)lPos + (int)lFileOffset;
                break;
    ...
    if ((!d.isXA10A) && (isMosaic) && (d.CSA.mosaicSlices < 1)) //See Erlangen Vida dataset - never reports "XA10" but mosaics have no attributes
    	printWarning("0008,0008=MOSAIC but number of slices not specified: \n");
...
*/
    ...
    if ((d.xyzDim[1] > 1) && (d.xyzDim[2] > 1) && (d.imageStart < 132)) {
    	printError("Conversion aborted due to corrupt file: \n");
    ...
    if ((!d.isLocalizer) && (isInterpolated) && (d.imageNum <= 1))
    	printWarning("interpolated protocol '%s' may be unsuitable for dwidenoise/mrdegibbs. \n", d.protocolName);
